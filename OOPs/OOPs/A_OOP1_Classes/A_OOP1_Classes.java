...
/*1.Clases are structures in java that allow to define a datatype the way we want. A more formal defination would be that classes are templates.

2.For example lets say we want to create a database that stores name, roll and marks of student. Now in a general case we would need to create 2 integer array and a string array to store the three entities and in that case they will not be together but in 3 different structures, something that we dont want. Here classes come in. In place of using traditional data sturctures like array, we can create our new data structure that can store name, roll and marks of the student together. We do this using classes, below see us defining student class.*/

public class A_OOP1_Classes{
    //3.Here we are creating new datastructure students which will store student data.
    class students{
        //4.Here we will define the properties of student. These are formally termed as instance variables or datamembers or attributes. All the variables inside the class are termed inside the class.
        String name;
        int roll;
        float marks;
        //5.Here we have completed creating a new datastructure student.

        //6.Lets now make a constructor. A constructor defines what occurs when an object of a class is created. Look how do we actually create a constructor.

        students(){};

        //7.Above is a constructor. Yes, the first rule of a creating a constructor is that its name should be the same as the name of the class. The second rule is that it does not has a return type. The constructor which we see above is constructor that takes no argument and doesn't modify any of the instance variables. Lets see another constructor.
    ...
    public static void main(String[] args) {
        /*11. When we talk about classes we need to also talk about objects. Objects are nothing but instance of classes. Now objects have 3 characteristics, state, identity and behavior.
        ...
        */
        ...
        /*12.Here we have just initialised the first student object refered as ashutosh. This reference will be stored in stack and pointing to null in heap since it has not been initialised yet. */

        
        /*13.The 'new' keyword dynamically allocates memory for an object(creates its instance in heap) & returns a reference to it(which is stored in variable in stack pointing to it in heap). This reference is, more or less, the address in memory of the object allocated by new. Thus, in Java, all class objects must be dynamically allocated.
        ...
        */
        ...
        //14.In the code above there will be a object created and reference to it will be stored with barun. When saha is equating to barun we see that in place of creating of copy of object, saha points to the same object. Hence changes made in either saha or barun will affect the same object.

        /*15.Since we are dicussing OOPs lets also discuss difference between objects and primitives.
