...
public class Main {
    public static void main(String[] args) {
        ...
        //In the code above we see that a Box reference is containing a BoxWeight object. Till this its all right. What we see here is we are able to intialise all the four datamembers but at the time we are trying to access the these datamembers, we are able to access all of them except for weight(un comment the code above to see). This is because reference variable type defines what all what all datamembers can we access. Since, Box contains of w,l,h we can access them but it isnt that case with weight, hence we are not able to access it. As for case with ability to initialise is because we are calling the constructor.
        //One more analogy that could be given here is that, the parent class knows about the existence of child hence a parent reference is able to hold a child object. 
